...
function fish_prompt
  ...
  set -l basic_prompt $green(whoami)$normal" at "$green(hostname -s)$normal" in "$blue$path" "
  ...
  if set -q VIRTUAL_ENV
    set -g virtualenv_name (basename $VIRTUAL_ENV)
  end
  ##################################################
  # Output
  if [ -n $status_info ]
  ...
  if set -q VIRTUAL_ENV
    echo -n -s "$normal(in $virtualenv_name)$prompt"
  else
    echo -n -s $prompt
  end
end
