...
complete -c omf -n "__omf.opt_is r rm remove"  -a (printf "%s " (omf.list_local_plugins) (omf.list_installed_themes))
complete -c omf -n "__omf.opt_is i install"    -a (printf "%s " (omf.list_db))
complete -c omf -n "__omf.opt_is t theme"      -a (printf "%s " (omf.list_themes))

complete -c omf -a list    -n "__omf.is_single_opt" -d "List local plugins"
complete -c omf -a install -n "__omf.is_single_opt" -d "Install one or more plugins"
complete -c omf -a theme   -n "__omf.is_single_opt" -d "List / Use themes"
complete -c omf -a remove  -n "__omf.is_single_opt" -d "Remove a theme or plugin"
complete -c omf -a update  -n "__omf.is_single_opt" -d "Update Oh My Fish!"
complete -c omf -a new     -n "__omf.is_single_opt" -d "Create a new plugin from a template"
complete -c omf -a submit  -n "__omf.is_single_opt" -d "Submit a plugin to the registry"
