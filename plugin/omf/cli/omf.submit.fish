...
function omf.submit -a name url -d "Submit a package to the registry"
  switch (dirname $name)
    ...
    case themes
    case "*"
      echo (omf::err)"Missing directory name: plugin/ or themes/"(omf::off) 1^&2
      return $OMF_INVALID_ARG
  end

  set -l plugin (basename $name)
  if not omf.util_valid_package $plugin
    echo (omf::err)"$plugin is not a valid package/theme name"(omf::off) 1^&2
    return $OMF_INVALID_ARG
  end
  ...
  if test -z "$url"
    ...
    if test -z "$url"
      echo (omf::em)"$plugin remote URL not found"(omf::off) 1^&2
      echo "Try: git remote add <URL> or see Docs#Submitting" 1^&2
      return $OMF_INVALID_ARG
    end
  else
    if test -e "$OMF_PATH/db/$name"
      echo (omf::err)"Error: $plugin already exists in the registry!"(omf::off) 1^&2
      return $OMF_INVALID_ARG
    else
      echo "$url" > $OMF_PATH/db/$name
      echo (omf::em)"$plugin added to the local "(dirname $name)" registry."(omf::off)
