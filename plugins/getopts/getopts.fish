...
#
#     1  The end of the options is reached. Also sends remaining operands
#        to stdout.
#
...
function getopts
  ...
  # Should always be called before returning 1.
  ...
  # Trim option string and collect required and optional options.
  if not set -q __getopts_optstr
    set -g __getopts_optstr (printf "$argv[1]" | tr '[:space:]' \n)
    set -e argv[1]
    ...
    if [ : = "$first_token" ]
      ...
      # We can safely remove the first character now.
      set __getopts_optstr (printf __getopts_optstr | cut -c2-)
    end

    # Collect options with optional and required option-arguments.
    function __getopts_collect_optargs
      for string in (printf "$__getopts_optstr" | tr " " \n)
    ...
    function __getopts_sanitize_argv
      for token in $argv
        switch $token
          case --\* # Skip!
          case -\*
            # Split each token in single characters with `.`
